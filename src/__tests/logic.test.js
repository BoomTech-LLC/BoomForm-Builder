import { getHiddenIds , conditionalLogic , getFieldValue , actionHandler , getUpdatableFields } from '../Helpers/logic';
...
describe('getUpdatableFields', () => {
    ...
    it('should return an array of ids with only "item" when logic has only "item"', () => {
      const logic = [
        { conditions: [{ id: 'field1', item: 'subitem1' }, { id: 'field2', item: 'subitem2' }] },
        { conditions: [{ id: 'field3', item: 'subitem3' }] }
      ];
      const result = getUpdatableFields({ logic });
      expect(result).toEqual(['field1.subitem1', 'field2.subitem2', 'field3.subitem3']);
    });
  });
...
describe('actionHandler', () => {
    ...
    it('should add id to hiddenFields.fields array when action is "show"', () => {
        const id = 'field1';
        const hidFields = actionHandler(id, 'show', '', true, hiddenFields, null, null);
        expect(hiddenFields.fields).toEqual(hidFields.fields);
      });
  
    it('should add id to hiddenFields.pages array when action is "show_page"', () => {
      const id = 'page1';
      const hidFields = actionHandler(id, 'show_page', '', true, hiddenFields, null, null);
    ...
    it('should return true when operator is "or" and action is "show"', () => {
      const id = 'field1';
      const result = actionHandler(id, 'show', 'or', true, hiddenFields, null, null);
      expect(result).toBe(true);
    });
  
    it('should return true when operator is "or" and action is not "show"', () => {
      const id = 'field1';
      const result = actionHandler(id, 'hide', 'or', true, hiddenFields, null, null);
      expect(result).toBe(true);
    });
  
    it('should return true when operator is "and" and action is "hide"', () => {
      const id = 'field1';
      const result = actionHandler(id, 'hide', 'and', false, hiddenFields, null, null);
      expect(result).toBe(true);
    });
  
    it('should return null for unknown action', () => {
      const id = 'field1';
      const result = actionHandler(id, 'unknown_action', '', true, hiddenFields, null, null);
...
describe('getFieldValue', () => {
    ...
    it('should return correct value for type "select"', () => {
      ...
      const field = { options: [{ key: 'option1', value: 'Value1' }, { key: 'option2', value: 'Value2' }] };
    ...
    it('should return label for selected value for type "singleChoice"', () => {
        const value = 'option2';
        const field = { options: [{ label: 'Option 1', value: 'option1' }, { label: 'Option 2', value: 'option2' }] };
    ...
    it('should return array of selected options for type "multipleChoice"', () => {
        ...
        const field = { options: [{ key: 'option1', label: 'Option 1' }, { key: 'option2', label: 'Option 2' }, { key: 'option3', label: 'Option 3' }] };
        const result = getFieldValue('multipleChoice', value, field, true);
        expect(result).toEqual([{ key: 'option1', label: 'Option 1' }, { key: 'option3', label: 'Option 3' }]);
      });
    ...
    it('should return hiddenFields with fields and pages based on logic conditions', () => {
        const logic = [
          ...
          },
          {
            action: 'hide_page',
            conditions: [
              { id: 'field3', value: 'value3', rule: 'is' }
            ],
            operator: 'and',
            id: 'action2',
            handlers: {}
