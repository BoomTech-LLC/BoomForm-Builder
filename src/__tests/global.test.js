    ...
    return {
      message: () =>
        pass
          ...
          : `Expected ${received} to be a string`,
      pass
    };
  }
});
...
describe('timeConversion(s)', () => {
  ...
  it("shouldn't receive invalid parameter", () => {
    expect(() => timeConversion(null)).toThrow(
      new TypeError("Cannot read property 'slice' of null")
    );
    expect(() => timeConversion(undefined)).toThrow(
      new TypeError("Cannot read property 'slice' of undefined")
    );
    expect(() => timeConversion(0)).toThrow(
      new TypeError('s.slice is not a function')
    );
...
describe('getNestedValue(values, index)', () => {
  it('should be called with correct params', () => {
    ...
    expect(getNestedValue(value, 'office.partners.0.address.city')).toEqual(
      'a'
    );
    ...
    expect(
      getNestedValue(value, 'office.partners.0.address.not_available_key')
    ).toEqual({});
  ...
  it("shouldn't be called with these values", () => {
    ...
    expect(() => getNestedValue(null, null)).toThrow(
...
describe('getRendableData(fields, hiddenFieldIds = {}, pagination = {}, currentPage)', () => {
  const { fields, logicIds: hiddenFieldIds, pagination, currentPage } = test;
  ...
  it('there is no pages provided in pagination', () => {
    ...
    const expectedResult = getPrintableFields(
      fields,
      hiddenFieldIds,
      pagination
    );
  ...
  it('provided nullish current page, returns only visible fields ids with an array', () => {
    const nullResult = getRendableData(
      fields,
      hiddenFieldIds,
      pagination,
      null
    );
    const undefinedResult = getRendableData(
      fields,
      hiddenFieldIds,
      pagination,
      undefined
