...
const Numbers = ({
  ...
  const handleValidationFields = (index, isValidateSubmit = false) => {
    ...
    pages.forEach((page, _index) => {
      const checkTouchedKey = Object.keys(touched).find((touchedKey) =>
        page.fields.find((_key) => touchedKey.includes(String(_key)))
      )
      const checkErrorsKey = Object.keys(errors).find((errorKey) =>
        page.fields.find((_key) => errorKey.includes(String(_key)))
      )
      if (index >= _index) {
      ...
      <div className='boomForm-paginationNumbers__content'>
        ...
        <SubmitButton
          handleValidationFields={() =>
            handleValidationFields(pages.length - 1, true)
          }
